<template>
	<div class="uk-container">
				...
				<div id="editFilterBlock" style="border-radius: 0px 0px 5px 5px; margin-top: -2px;">
					<div>
						...
						<div class="uk-grid-small margin-small-top" uk-grid>
							<div class="filter-label-container"><p class="filter-label">Плательщик</p></div>
							<div class="uk-width-expand">
								<input type="text" class="uk-input main-input" v-model="rawFilter.title">
							</div>
						</div>
						<div class="uk-grid-small margin-small-top" uk-grid>
							<div class="filter-label-container"><p class="filter-label">ИНН</p></div>
							<div class="uk-width-expand">
								<input type="text" class="uk-input main-input" v-model="rawFilter.INN">
							</div>
						</div>
		...
		<div v-for="payer, index in payers" :key="index">
			<router-link :to="{ name: 'editpayer', params: {id: payer.id}}" style="text-decoration: none !important;">
				<div class="preset-container uk-grid-collapse uk-margin-small-bottom" uk-grid>
					<div class="uk-width-expand">
						<p class="common-data" style="margin-right: 5px; overflow: hidden;"><span style="display: block; width: 10000px;">{{ payer.title }}</span></p>	
					</div>
					<div>
						<p class="key-number">{{ payer.phoneNumber }}</p>
					</div>
					<div>
...
export default {
	...
	methods: {
		...
		showMore() {
			this.loading = true;
			PayerService.getAllPayers(this.page, this.perPage, { createdAt: -1 }, this.filter).then(resp => {
				this.payers = this.payers.concat(resp.data.payers)

				if(resp.data.payers.length == 0){
					if(this.page == 1){
						window.UIkit.notification("Плетьщики отсутствуют")
					} else {
						window.UIkit.notification("Все плательщики уже выведены")
					}
				}

				if(resp.data.totalItems == this.payers.length){
		...
		processFilter(){
			...
			if(this.rawFilter.INN !== ''){
				this.filter.INN = this.rawFilter.INN
				this.filterFieldsCounter += 1
			}
			window.UIkit.toggle('#editFilterBtn').toggle()
